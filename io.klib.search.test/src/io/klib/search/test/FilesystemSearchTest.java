...
public class FilesystemSearchTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		BundleContext ctx = FrameworkUtil.getBundle(TestdataSetupBundle.class)
				.getBundleContext();
		TESTDIR = ctx.getDataFile(TestdataSetupBundle.TESTDATA);
		assertTrue(TESTDIR.exists());

		ServiceReference ref = ctx.getServiceReference(SearchFilesystem.class
				.getName());
		assertNotNull(ref);
	...
	public void testFlatFileSearch() throws Exception {
		...
		Set<URI> found = searchFS.find(TESTDIR, ".*\\.txt", null,
				(SearchOption[]) null);
	...
	public void testRecursiveFileSearch() throws Exception {
		Set<URI> expected = new HashSet<URI>();
		// //@formatter:off
		expected.addAll(testfiles(new String[] { "file1.txt",
				"/dirA/dirJ/file1.txt", "/dirA/dirJ/file2.txt",
				"/dirA/dirJ/file3.txt", "/dirA/dirK/file1.txt",
				"/dirB/file2.txt" }));
		// //@formatter:on
	...
	public void testArchiveFileSearch() throws Exception {
		...
		// //@formatter:off
		expected.addAll(testfiles(new String[] { "file1.txt",
				"/dirA/dirJ/file1.txt", "/dirA/dirK/file1.txt" }));
		...
		Set<URI> found = searchFS
				.find(TESTDIR, ".*file1.txt", null, new SearchOption[] {
	...
	public void testArchiveContentSearch() throws Exception {
		...
		Set<URI> found = searchFS.find(TESTDIR, ".*\\.txt",
				".*new\\r\\nline.*", new SearchOption[] { SearchOption.RECURSE,
	...
	public void testArchiveContentDotAllSearch() throws Exception {
		...
		Set<URI> found = searchFS
				.find(TESTDIR,
						...
						new SearchOption[] { SearchOption.RECURSE,
								SearchOption.ARCHIVE });
		assertEquals(expected, found);
	}
	...
	public void testRecursiveNegatedFileSearch() throws Exception {
		Set<URI> expected = new HashSet<URI>();
		// //@formatter:off
		expected.addAll(testfiles(new String[] { "file1.txt", "zip_dirA.zip",
				"zip_dirB.zip", "/dirA/", "/dirA/dirJ", "/dirA/dirJ/file1.txt",
				"/dirA/dirJ/file3.txt", "/dirA/dirK/", "/dirA/dirK/file1.txt",
				"/dirB/", "/dirB/file2.txt", "/dirC/",
				"/dirC/osgi.core-4.0.1.jar", "/dirC/zip_dirB.zip" }));
		// //@formatter:on

		Set<URI> found = searchFS.find(TESTDIR,
				"^(?!(.*/dirA/dirJ/file2.txt)).+$", null, SearchOption.RECURSE);
		assertEquals(expected, found);
	}
